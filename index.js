...
let artistLookup = array[number];
return `the artist at index ${artistLookup.id} is ${artistLookup.name}`;
}
...
function lotsOfArt(array) {
  let experiencedArtist = [];
  for(let i = 0; i < array.length; i++){
    if(array[i].paintings > 100){
      experiencedArtist.push(array[i].name)
    }
  }
  return experiencedArtist;
}
